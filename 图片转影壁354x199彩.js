...
let imgPath = dialogs.rawInput("选择图片的路径", config.get("defaultImgPath","/storage/emulated/0/8.jpg"));
let start = dialogs.input("选择起始列数", "0");
let end = dialogs.input("选择终止列数", "100");
...
if(!useCustomPos){
    if (device.height == 3120 && device.width == 1440) {
        ...
        //请在修改结束后删掉这个 'exit();'
        exit();
        var pixelWidth = 2; //用比最小笔刷宽度大一点点的宽度点一个点，这个点的直径
...
//现在颜色顺序会变化了！所以自动检测顺序，黑色在最上方，因为往上拉的时候有一定几率卡住，所以画前先拉到最上
...
function switchColor(colId, needSwipe) { //更换当前笔刷颜色
    if (needSwipe) {
        while( images.captureScreen().pixel(colorSelecterX, colorSelecterY[0]) !=  colorTable[0] ){
...
function execAlgo0(){
    ...
    for (var i = start+1; i <= end; i++) {
        for (var j = 1; j <= pixelCountY; j++) {
            let searchx = (i - 1);
            let searchy = (j - 1);
            col = colors.red(img.pixel(searchx, searchy));
            let curcolId = findNearestColor(img.pixel(searchx, searchy), prevCol, prevColId);

            // prevCol = img.pixel(searchx, searchy);
            // if (colId==0)continue; //跳过白色
            if (curcolId != prevColId) {
                var needSwipe = 0;
                // if ((colId <= 4 && colId >= 0 && prevColId <= 4 && prevColId >= 0) || (colId <= 7 && colId >= 5 && prevColId <= 7 && prevColId >= 5)) {} else { //两个颜色不在同一页
                needSwipe = 1;
                
                prevColId = curcolId;
                switchColor(curcolId, needSwipe);
            };
            // swipe(printAreaBegin[0] + i * pixelGap, printAreaBegin[1] + j * pixelGap,printAreaBegin[0] + i * pixelGap, printAreaBegin[1] + j * pixelGap+2,30);

            press(printAreaBegin[0] + 3*(i * pixelGap), printAreaBegin[1] + 3*(j * pixelGap), 1);
            //sleep(200);
        };
